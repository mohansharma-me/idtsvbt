<div class="sidebar-right">
	...
	<hr/>
		<div align=left style="padding-left:20px;height:auto" class="msgs">
			<li><a href="accounts.php">Profile</a></li>
			<li><a href="?tab=settings">Settings</a></li>
			<li><a href="?tab=attendance">Attendance</a></li>
			<li><a href="?tab=result">Result</a></li>
			<li><a href="?tab=messages">Messages</a></li>
			<li><a href="?tab=notice">Notice Board</a></li>
			<li><a href="?tab=fees">Fees details</a></li>
			<li><a href="?tab=timetable">Time Table</a></li> 
...
<?php
                                ...
                                if(cerr()) {
                                    if(mysql_affected_rows()==0) {
                                        ...
                                        if($password==$row[0]) {
                                            ...
                                            if(cerr()) {
                                                $delete_student=mysql_query("delete from student where acc_id='$acc_id'");
                                                if(cerr()) {
		...
		echo "<br/><u><h2>Attendance Details</h2></u>";
        mysql_select_db("idtsvbt_db_faculty");
        $search=mysql_query("select enrollno from student where acc_id='$acc_id'");
        if(cerr()) {
            if(mysql_affected_rows()==0) {
                ...
                if(cerr()) {
                        if(mysql_affected_rows()==0) {
                                echo "<br/><center><div id='error' style='width:80%'>Sorry, your entered enrollment no may be incorrect or not in database!</div></center>";					
                        } else {
                                $row=mysql_fetch_row($search);
                                ?>
                                <br/>
		...
		echo "<br/><u><h2>Result Details</h2></u><br/>";
        mysql_select_db("idtsvbt_db_faculty");
        $search=mysql_query("select branch,sem,enrollno from student where acc_id='$acc_id'");
        if(cerr()) {
            if(mysql_affected_rows()==0) {
        ...
        if(isset($_GET['action'])) {
            if($_GET['action']=="create") {
                if(isset($_POST['ruser']) && isset($_POST['msg'])) {
                    if(carr($_POST)) {
                        $ruser=strtolower($_POST['ruser']);$msg=strtolower($_POST['msg']);
                        if($ruser==strtolower($_SESSION['username'])) {
                            echo "<font color='red'><div>You cant send message yourself!</div></font>";
                        } else {
                            mysql_select_db("idtsvbt_db_faculty");
                            $search=mysql_query("select username from login where lower(username)='$ruser'");
                            if(cerr()) {
                                if(mysql_affected_rows()==0) {
                ...
                echo "<table>";
                echo "<tr><td style='text-align:right'>Username :- </td><td><input type='text' name='ruser' value='".(isset($_GET['user'])?$_GET['user']:"")."' style='width:600px'/></td></tr>";
            ...
            echo "<br/><table class='mytable' style='width:100%' cellspacing='0'>";
        ...
        mysql_select_db("idtsvbt_db_faculty");
        $search=mysql_query("select branch,sem from student where acc_id='$acc_id'");
        if(cerr()) {
            $row=mysql_fetch_row($search);
            ...
            if(cerr()) {
                if(mysql_affected_rows()==0) {
                    echo "<br/><div id='warning'>Sorry there is zero items in your branch's notice board!</div><br/>";
                } else {
                    $data['current']=$page;
        ...
        mysql_select_db("idtsvbt_db_faculty");
        $getenroll=mysql_query("select enrollno from student where acc_id='$acc_id'");
        if(cerr()) {
            ...
            if(cerr()) {
                echo "<br/>";
                if(mysql_affected_rows()==0) {
                    echo "<div id='warning'>Sorry there is no fees details for you!!</div>";
                } else {
                    $row=mysql_fetch_row($search);
        ...
        if(cerr()) {
            ...
            if(cerr()) {
                if(mysql_affected_rows()==0) {
                    echo "<br/><div id='warning'>Sorry there is no record for your branch and sem!</div><br/>";
                } else {
    ...
    mysql_select_db("idtsvbt_db_faculty");
    $search=mysql_query("select * from student where acc_id='$acc_id'");
    if(cerr()) {
        $row=mysql_fetch_row($search);
    } else {
        ms_err("1");
    ...
    if(isset($_GET['action'])) {
        if($_GET['action']=="edit_profile") {
            if(isset($_POST['fname']) && isset($_POST['email']) && isset($_POST['branch']) && isset($_POST['sem']) && isset($_POST['enrollno']) && isset($_POST['mobile'])) {
                if(carr($_POST)) {
                    $fname=strtolower($_POST['fname']);$email=strtolower($_POST['email']);$branch=strtolower($_POST['branch']);$sem=strtolower($_POST['sem']);$enrollno=strtolower($_POST['enrollno']);$mobile=strtolower($_POST['mobile']);
                    if(is_numeric($mobile) && is_mail($email) && is_numeric($enrollno)) {
                        $update=mysql_query("update student set fname='$fname',email='$email',branch='$branch',sem='$sem',enrollno='$enrollno',mobile='$mobile' where id='$acc_id'");
                        if(cerr()) {
                            echo "<br/><div id='done'>Your details updated successfully.</div><br/>";
                            $search=mysql_query("select * from student where acc_id='$acc_id'");
                            $row=mysql_fetch_row($search);
                        } else {
                            ms_err("1");
                    ...
                    echo "<br/><div id='error'>You have to fill all the field of the web form to update your details!</div><br/>";
                }
            }
    ?>
    <form method="post">
    ...
    <tr class="head"><td style="width:20%;text-align: right"><b>Mobile :-</b></td><td><input type="text" name="mobile" value="<?=ucwords($row[7])?>" style="width:500px" /></td></tr>
    ...
    <tr class="head"><td style="width:20%;text-align: right"><b>Mobile :-</b></td><td><?=ucwords($row[7])?></td></tr>
    </table><br/>
        <div class="linkbox">
